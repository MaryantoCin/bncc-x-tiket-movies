...
const MovieDetail = ({ movieData, creditsData, session_id, user_data }) => {
    ...
    if (session_id) {
      ...
      if (result.success) {
        showToast(
          ...
          "Success!",
          `Film berhasil ${
            isInFavoriteList ? "dihapus dari" : "ditambahkan ke dalam"
          } list favorit`,
          "success",
          2000,
          true
        ...
        showToast(
          ...
          `Gagal ${
            isInFavoriteList
              ? "menghapus film dari"
              : "menambahkan film ke dalam"
          } list favorit. Mohon coba lagi....`,
          "error",
          2000,
          true
        );
  ...
  const getUserFavoriteMovies = async () => {
    await api
      .get(`/account/${user_data.id}/favorite/movies?session_id=${session_id}`)
      .then((res) => {
...
export async function getServerSideProps(context) {
  const movieData = await api
    .get(`movie/${context.query.id}`)
    .then((res) => res.data);
  const creditsData = await api
    .get(`movie/${context.query.id}/credits`)
    .then((res) => res.data);
